...
function switchPage(page) {
  ...
  // 三级页（隐藏底部 tab 栏）：自动化 / 工具箱及其工具页 / 文件整理及其子页
  var THIRD_LEVEL = ['auto', 'toolbox', 'toolbox-magnet', 'tidy', 'tidy-tasks', 'tidy-chat', 'tidy-rules'];
  if (tb) tb.style.display = (THIRD_LEVEL.indexOf(page) >= 0) ? 'none' : '';
  if (page === 'search'){
...
function updateSubtitleBtn(dvdId){
  ...
  // 免费 / 中级不显示字幕按钮（字幕仅满级可用，否则点了只会弹「暂不可用」）；
  // 有番号（如 JAV）的影片也不显示：字幕按标题匹配，仅无番号影片适用。
  ...
  if (btn) btn.style.display = (isFull && !hasDvdId) ? '' : 'none';
}
...
/* 按档位显隐的按钮：自动化页「字幕搜索」浮动按钮 + 详情页字幕按钮；仅满级可见 */
function updateAutoFabTier(){
  var isFull = (state.tier || '') === 'full';
  var sub = document.getElementById('autoSubBtn');
  if (sub) sub.style.display = isFull ? '' : 'none';
  // 若详情页正开着，详情页字幕按钮一并按新档位刷新（带上当前影片番号，JAV 仍不显示）
  if (typeof currentDetailFilm !== 'undefined' && currentDetailFilm){
    updateSubtitleBtn((currentDetailFilm.data || {}).dvdId);
  }
}
function updateActivationStatus(){
  updateAutoFabTier();   // 档位变化的统一出口：顺手刷新按档位显隐的按钮
...
/* 纯客户端动作（如 115 整理，服务端不参与）向服务端自报一次，让 /quota 的真实剩余同步 */
function reportQuotaUse(field){
  getActivationCode().then(function(code){
    var base = state.magnetWorker || DEFAULT_WORKER;
    return fetch(base + '/quota/report?field=' + encodeURIComponent(field) + '&code=' + encodeURIComponent(code || ''), { cache: 'no-store' });
  }).then(function(){ renderQuotaInfo(); }).catch(function(){});
}
...
/* ===== 115 自动化：离线 → 建目录 → 移视频 → 改名 → 清理 =====
   ...
   本文件只留 API 调用（auto115Post/ListDir…）与 UI 渲染薄层。 */
...
function auto115EnsureLibraryDoc(){
  ...
  return idbGet('kv', auto115Key('tbm-library')).then(function(v){
    ...
    /* v338：磁力库多磁力旧任务是 6 步 multi 表，按「仅下载」语义清洗成 2 步（已指定整理目标的保留整理步骤） */
    var libChanged = false;
    (auto115LibraryDoc.tasks || []).forEach(function(tt){
      if (auto115NormalizeTaskSteps(tt, tt.tbmType === 'tv')) libChanged = true;
    });
    if (libChanged) auto115Save(auto115LibraryDoc);
...
/* 存量任务步骤清洗（v338）：多磁力任务按自己的步骤表过滤 + 重排。
   背景：旧版 status() 用 getStep 高频读步骤，会把表里没有的 key 以 idle 幽灵形式
   append 进 t.steps（剧集多磁力任务被补出 mkdir2/move2 两个永远空转的节点）。
   返回 true 表示有清洗（调用方负责保存）。 */
function auto115NormalizeTaskSteps(tt, isTv){
  if (!tt || !tt.steps || !tt.steps.length) return false;
  if (!Auto115Core.isMultiMagnet(tt)) return false;
  var table;
  if (tt.tbm){
    /* 磁力库多磁力：仅下载（2 步）；已指定整理目标（tbmType）则保留其整理步骤 */
    table = (!tt.tbmType) ? null : (tt.tbmType === 'tv' ? Auto115Core.STEPS_TV : Auto115Core.STEP_DEFS);
  } else {
    table = isTv ? Auto115Core.STEPS_TV : Auto115Core.STEPS_MULTI;
  }
  if (!table){
    var keep = {};
    Auto115Core.STEPS_TBM.forEach(function(s){ keep[s.key] = 1; });
    var out0 = tt.steps.filter(function(s){ return keep[s.key]; });
    if (out0.length === tt.steps.length) return false;
    tt.steps = out0; return true;
  }
  var order = {};
  table.forEach(function(s, i){ order[s.key] = i; });
  var out = tt.steps.filter(function(s){ return order[s.key] != null; });
  out.sort(function(a, b){ return order[a.key] - order[b.key]; });
  if (out.length === tt.steps.length && out.every(function(s, i){ return s === tt.steps[i]; })) return false;
  tt.steps = out; return true;
}
    ...
    if (v && v.tasks){
      ...
      for (var i = 0; i < v.tasks.length; i++){
        ...
        if (tt.fv !== AUTO115_FLOW_VERSION){
          ...
          tt.steps = Auto115Core.isMultiMagnet(tt) ? Auto115Core.newSteps('multi', isTv) : auto115NewSteps();
...
function auto115MovieLayout(doc){ return Auto115Core.movieLayout(doc || auto115Doc || {}); }
...
function auto115TaskDoc(t){
  ...
  if (d) return d;
  if (t && t.filmId){
    if (auto115ExecDoc && auto115ExecDoc.filmId === t.filmId) return auto115ExecDoc;
    if (auto115Doc && auto115Doc.filmId === t.filmId) return auto115Doc;
  }
  return auto115Doc;
}
...
function renderAuto115(){
  ...
  var topT = document.getElementById('autoTopTitle');
  if (topT) topT.textContent = (auto115Doc.type === 'tv') ? '自动化 · TV' : '自动化';
...
function auto115FindSameMagnet(magnet){
  ...
  var s = String(magnet || '').trim().toLowerCase();   /* ed2k 无 btih，退整串比对 */
  if ((!h && !s) || !auto115Doc) return null;
  var ts = auto115Doc.tasks || [];
  for (var i = 0; i < ts.length; i++){
    var x = ts[i];
    if (!x || x.aborted || x.tidy) continue;
    var xm = String(x.magnet || '').trim().toLowerCase();
    if (h ? (auto115Btih(x.magnet || '') === h) : (xm === s)) return x;
  }
  return null;
}
...
function auto115DispatchStep(t, key){
  ...
  if (Auto115Core.isMultiMagnet(t)){
    ...
    /* v338：剧集多磁力走完整 8 步——mkdir2（建季夹）/rename（只改名）/move2（只移入）各自是真节点；
       电影多磁力没有这三步，rename 合并执行。 */
    if (key === 'mkdir2') return auto115StepTvMkdir2Multi(t);
    if (key === 'rename') return auto115StepRenameMulti(t);
    if (key === 'move2') return auto115StepMove2Multi(t);
...
function auto115MovieVideoName(videoName, doc){
  /* 原文件名带清晰度标识（1080p/2160p/4k…）→ 追加在年份后面：标题.原始标题.年份.1080p */
  return Auto115Core.movieVideoName(doc || auto115Doc, videoName ? Auto115Core.qualityTag(videoName) : '');
}
...
function auto115ApplyRenames(t, jobs, targetCid, mode){
  /* mode（v338）：null = 移动+改名合并执行（电影多磁力）；'rename' = 只改名（剧集多磁力改名节点）；
     'move' = 只移动（剧集多磁力移入节点——改名不改 fid，直接按 job.pid 移进季夹） */
...
/* 剧集临时夹里的「遗留文件」：tvPlan 认不出集号但被保留的视频/字幕（v256 起不删不改名）。
...
function auto115StepMove(t){
  ...
  return auto115ListDir(t.offlineDirCid).then(function(list){
    ...
    var keep;
    var secondPick = null;  // 次清晰度版本（v269）：跟随主视频一起改名（标题.年份.720p），最多 1 个
    ...
    if (strong.length){
      ...
      /* 强命中不止一个时分两种情况：
         ...
         ② 没带分碟标记、且清晰度混档 → 这不是分碟，是一部片的多个版本：
            最高清当主视频，次清晰度跟随改名保留（「最多保留两个清晰度，选最清晰的」），
      ...
      /* 弱命中也能配对次清晰度（v269 修「zjz6 夹里 720p 没识别」）：视频名是拼音缩写、命中不了标题时，
         主视频按体积当选；此时看其余视频——名字主干与主视频一致（去掉清晰度/水印后相同）且清晰度
         明确更低 → 取最清晰的一档当次清晰度版本，跟随主视频一起改名搬出，不再当「其他视频」原地留着 */
      if (keep.length === 1){
    ...
    /* 次清晰度版本：记下 fid/名字/体积，rename 时跟随主视频一起改名搬出（标题.年份.720p），不算「其他视频」 */
    ...
    for (var i = 0; i < list.length; i++){
      ...
      if (it.fid){
        ...
        if (auto115IsVideoName(nmz) && !EXCLUDE.test(nmz)){
          ...
          keptVids++;
          /* 记下 fid/名字/体积：rename 收尾看到夹里还有这类视频就**不删临时夹**（v267 约定：
             其他视频留在原文件夹不动，只搬主视频） */
...
function auto115FindFlatVideo(doc){
  var D = doc || auto115Doc;
  var titles = auto115TidyTitles(D);
  if (!titles.length) return Promise.resolve('');
  if (D && D.flatVideoFid) return Promise.resolve(String(D.flatVideoFid));
  return auto115ListDir(C115_DEFAULT_DIR_CID).then(function(list){
    var vids = (list || []).filter(function(it){ return it && it.fid && auto115IsVideoName(it.n || it.name || ''); });
    var best = '', bs = 0;
    for (var i = 0; i < vids.length; i++){
      var s = Auto115Core.tidyScore(vids[i].n || vids[i].name || '', titles, D && D.year);
...
function auto115PullFlatVideo(t){
  ...
  t._pulled = true;
  return auto115FindFlatVideo(auto115TaskDoc(t)).then(function(vfid){
    ...
    return auto115Post('https://webapi.115.com/files/move', 'fid=' + encodeURIComponent(vfid) + '&pid=' + encodeURIComponent(t.offlineDirCid)).then(function(res){
      ...
      var D = auto115TaskDoc(t);
      if (D){ D.flatVideoFid = ''; D.flatVideoName = ''; }
      auto115Set(t, 'move', 'running', '已把云下载里的视频收进文件夹');
      return true;
    });
  }).catch(function(){ return false; });
}
/* 合集保护保下来的其他视频（赌神2 等）按约定**留在原文件夹不动**——只搬主视频，
   不把不相关视频一起搬出来。临时夹里还有这类视频时，rename 收尾就不再删除临时夹
   （删了它们就跟着进回收站）；夹里只剩主视频/已清空的才照常删。 */
function auto115HasKeptOthers(t){ return !!(t.keptOthers && t.keptOthers.length); }
/* 次清晰度版本（同片的 720p/1080p 双版本）的改名任务：影片 → 标题.年份.720p（清晰度取自原文件名），
   AV → 番号.720p。跟随主视频一起搬出。 */
function auto115SecondVersionJob(t){
  ...
  var D = auto115TaskDoc(t);
  if (D && D.dvdId){
    return { fid: sv.fid, name: D.dvdId + (sq ? ('.' + sq) : '') + e, orig: sv.name, size: sv.size || 0 };
  }
...
function auto115StepRename(t){
  ...
  var allJobs = jobs.concat(subJobs).concat(svJobPlain ? [svJobPlain] : []);
...
function auto115StepRenameFlat(t){
  var ext = (/\.[a-z0-9]+$/i.exec(t.videoName || '') || ['.mp4'])[0];
  var baseName = auto115MovieVideoName(t.videoName, auto115TaskDoc(t));
  if (!baseName){ auto115Set(t, 'rename', 'fail', '缺少名称信息，没法自动改名'); auto115Finish(t); return Promise.resolve(null); }
  var keep = (t.keepFids && t.keepFids.length) ? t.keepFids.slice() : (t.videoFid ? [t.videoFid] : []);
  if (!keep.length){ auto115Set(t, 'rename', 'fail', '未定位到视频文件，请重试'); auto115Finish(t); return Promise.resolve(null); }
  var multi = keep.length > 1;
  ...
  return auto115ListDir(C115_DEFAULT_DIR_CID).then(function(list){
    ...
    /* 次清晰度版本跟随主视频一起改名搬到根目录（标题.年份.720p） */
...
/* 多磁力整理计划（v338）：穿透 + 建夹 + 目标名单点计算，结果按任务 id 缓存。
   mkdir2/rename/move2 三个节点共享同一份 jobs；页面重载后缓存为空会重算
   （穿透基于源夹当前内容，幂等：已改名的会跳过改名，已移走的不再出现）。 */
var auto115MultiPlans = {};
function auto115MultiPlan(t){
  var c = auto115MultiPlans[t.id];
  if (c) return Promise.resolve(c);
  var ms = Auto115Core.magnets(t).filter(function(m){ return (m.dirCid || m.noFolder) && m.state === 'done'; });
  if (!ms.length) return Promise.resolve(null);
  var isTv = auto115TaskIsTv(t);
  var doc = auto115TaskDoc(t) || {};
...
function auto115StepTvMkdir2Multi(t){
  ...
  return auto115MultiPlan(t).then(function(plan){
    ...
    if (!plan.jobs.length){ auto115Set(t, 'mkdir2', 'ok', '无需新建季文件夹'); return auto115StepRenameMulti(t); }
    /* 从目标名里收出涉及的季号，给节点一个可读结果（单季不分夹，平铺在剧集根） */
    var seasons = {};
    plan.jobs.forEach(function(j){ var mm = /\.S(\d{2})E\d{2}/.exec(j.name || ''); if (mm) seasons[Number(mm[1])] = true; });
    var sk = Object.keys(seasons).map(Number).sort(function(a, b){ return a - b; });
    auto115Set(t, 'mkdir2', 'ok', sk.length > 1
      ? '季文件夹已就绪：' + sk.map(function(s){ return 'S' + auto115Pad2(s); }).join('、')
      : '文件夹已就绪');
    return auto115StepRenameMulti(t);
  }).catch(function(e){
    auto115Set(t, 'mkdir2', 'fail', (e && e.message) ? e.message : '准备季夹失败'); auto115Finish(t); return null;
  });
}
/* 多磁力「修改视频名称」节点：
...
function auto115StepRenameMulti(t){
  ...
  var isTv = auto115TaskIsTv(t);
  return auto115MultiPlan(t).then(function(plan){
    if (!plan){ auto115Set(t, 'rename', 'skip', '没有可整理的磁力'); auto115Finish(t); return Promise.resolve(null); }
    if (!plan.jobs.length){ auto115Set(t, 'rename', 'ok', '无需改名'); auto115Finish(t); return null; }
    if (isTv){
    ...
    return auto115ApplyRenames(t, plan.jobs, null).then(function(){
      ...
      return auto115MultiCleanEmptyDirs(srcDirs).then(function(){
        auto115Set(t, 'rename', 'ok', '已整理 ' + plan.jobs.length + ' 条磁力');
        auto115Finish(t); return null;
      });
    }).catch(function(e){
      auto115Set(t, 'rename', 'fail', (e && e.message) ? e.message : '改名失败'); auto115Finish(t); return null;
...
function auto115StepCleanup(t){
  if (auto115TaskIsTv(t)){
    ...
    var body = 'fid=' + encodeURIComponent(t.offlineDirCid) + '&file_name=' + encodeURIComponent(showTitle);
  ...
  if (t.noFolder){
    ...
    var dirName = lay.name || (auto115TaskDoc(t) && auto115TaskDoc(t).filmTitle) || '';
  ...
  if (t.external){
    ...
    var rawName = ((auto115TaskDoc(t) && (auto115TaskDoc(t).filmTitle || auto115TaskDoc(t).dvdId)) || t.offlineDirName || '').trim();
  ...
  var body2 = 'fid=' + encodeURIComponent(t.offlineDirCid) + '&file_name=' + encodeURIComponent(newName);
  return auto115Post('https://webapi.115.com/files/edit', body2).then(function(res){
    var d = res.d || {};
    if (res.ok && (d.state === true || d.errno === 0)){
...
function auto115UploadDirName(doc){
  var D = doc || auto115Doc;
  return sanitizeName((D && D.filmTitle) || '') || '';
}
...
function auto115MoveFlatVideoInto(t){
  ...
  if (auto115MovieLayout(auto115TaskDoc(t)).folder) return Promise.resolve(false);   // AV / 之前传过 NFO：本来就在文件夹里，不用动
  return auto115FindFlatVideo(auto115TaskDoc(t)).then(function(vfid){
    if (!vfid) return false;
...
function auto115StepUploadFiles(t){
  ...
  return loadFilm((auto115TaskDoc(t) || {}).filmId).then(function(film){
    ...
    var base = _upDoc.dvdId || sanitizeName(_upDoc.filmTitle || '') || 'movie';
    ...
    return Promise.all(bakeJobs).then(function(){
      ...
      return next().then(function(){
        ...
        return auto115MoveFlatVideoInto(t).then(function(moved){
          ...
          var _upDoc2 = auto115TaskDoc(t);
          if (_upDoc2) _upDoc2.nfoUploaded = auto115Now();
...
function auto115AddFromOp(){
  ...
  auto115EnsureDoc().then(function(doc){
    ...
    var t = {
      id: 't' + auto115Now().toString(36) + Math.random().toString(36).slice(2, 6),
      magnet: magnet, magnetTitle: title || auto115OfflineTitle(magnet),
...
function auto115OpenTidy(){
  ...
  ensure115Cookie().then(function(ck){
    ...
    return auto115EnsureDoc().then(function(){
      ...
      /* V5 修复：文件夹与散装视频同场竞技，分高者赢（原来文件夹优先，散装视频哪怕 100 分也不看） */
      var bestF = (m.ok && m.best) ? m.best : null;
      var bestV = (mv.ok && mv.best) ? mv.best : null;
      if (bestF && (!bestV || bestF.score >= bestV.score)) return auto115StartTidy(bestF);
      if (bestV) return auto115StartTidy(bestV);
...
function deleteFilm(id){
  ...
  if (auto115ExecDoc && auto115ExecDoc.filmId === id) auto115ExecDoc = null;
  return Promise.all([
...
function toolboxOpen(id){
  ...
  switch (hit.tool.page){
    ...
    case 'tidy115': openTidy(); return;                 // T4 文件整理（M4）：选文件夹 → AI 整理 / 规则整理
    default: showToast('即将上线', 'success'); return;  // T2 / T5 在 M5-M6 接入
  }
}
/* ============================================================
 * 文件整理（M4）：入口页 / 任务列表 / AI 对话 / 规则页
 * ------------------------------------------------------------
 * 纯逻辑（水印清洗 + 规则注册表）在 src/tidy-core.js；本处只做
 * 页面渲染、115 交互（列目录 / 改名）与本地持久化。
 * 会话与任务都存 localStorage：条目少、结构简单，不值得上 IndexedDB。
 * ============================================================ */
var TIDY_FOLDER_KEY = 'nfo_tidy_folder';       // 上次选中的文件夹
var TIDY_WM_KEY = 'nfo_tidy_watermarks';       // 用户编辑过的水印表
var TIDY_TASKS_KEY = 'nfo_tidy_tasks';         // 任务列表（AI 会话 + 规则执行）
var TIDY_CHAT_PREFIX = 'nfo_tidy_chat_';       // 单条会话消息：nfo_tidy_chat_<id>
var TIDY_TASK_MAX = 50;

var tidyState = {
  folder: null,      // { cid, name, path }
...
function tidyStore(key, val){ try { localStorage.setItem(key, JSON.stringify(val)); } catch (e) {} }
function tidyTimeFmt(ts){
  var d = new Date(ts || 0), p = function (n) { return n < 10 ? '0' + n : '' + n; };
  return (d.getMonth() + 1) + '-' + p(d.getDate()) + ' ' + p(d.getHours()) + ':' + p(d.getMinutes());
}
function tidySheetOpen(maskId, sheetId){
  var m = document.getElementById(maskId), s = document.getElementById(sheetId);
  if (m) m.classList.add('show');
  if (s) s.classList.add('show');
}
function tidySheetClose(maskId, sheetId){
  var m = document.getElementById(maskId), s = document.getElementById(sheetId);
...
function openTidyFolderPicker(){
  ensure115Cookie().then(function (ck){
    ...
    tidyState.pickCache = {};   /* 会话内页缓存随每次打开重置，避免用到旧数据 */
    tidyPickLoad();
...
function tidyPickListHtml(items, pick, err){
  ...
  var sorted = items.slice().sort(function (a, b){
    var da = (a && a.cid && !a.fid) ? 0 : 1, db = (b && b.cid && !b.fid) ? 0 : 1;
    return da - db;
  });
...
/* 真正把一页画上去（total/hasMore 判定 + 列表 + 翻页条） */
function tidyPickPaint(pick, page, items, count, err){
  ...
  pick.hasMore = pick.noMore ? false
    : (count ? (page + 1) * TIDY_PICK_PAGE_SIZE < count : items.length >= TIDY_PICK_PAGE_SIZE);
  listEl.innerHTML = tidyPickListHtml(items, pick, err);
  listEl.scrollTop = 0;
...
function tidyPickLoad(){
  ...
  /* 会话内页缓存：看过的文件夹/页码即时回显，进退与翻页零等待（卡顿的主因是每次都白屏等网络） */
  var cache = tidyState.pickCache || (tidyState.pickCache = {});
  var c = cache[pick.cid];
  if (c && c.pages && c.pages[page]){
    tidyPickPaint(pick, page, c.pages[page], c.count, '');
    return;
  }
  /* 没看过才真正去取：旧内容保留只调暗（不再整页换成「正在读取…」，消除闪一下的卡顿感） */
  listEl.classList.add('loading');
  tidyPickFetchPage(pick.cid, page).then(function (r){
    cache[pick.cid] = { pages: (c && c.pages) || {}, count: r.count };
    cache[pick.cid].pages[page] = r.items;
    /* 翻过头了（末页刚好被翻走）→ 退回上一页，并记住没有下一页 */
    if (!r.items.length && page > 0){
      delete cache[pick.cid].pages[page];
      pick.page = page - 1; pick.noMore = true; tidyPickLoad(); return;
    }
    tidyPickPaint(pick, page, r.items, r.count, '');
  }).catch(function (e){
    tidyPickPaint(pick, page, [], 0, (e && e.message) || '网络错误');
  }).then(function (){
...
function tidyPickHere(){
  var pick = tidyState.pick;
  if (!pick) return;
  var path = pick.stack.map(function (s) { return s.name; }).concat([pick.name]).join('/');
  tidyState.folder = { cid: pick.cid, name: pick.name, path: pick.name === '根目录' ? '网盘根目录' : path };
  tidyStore(TIDY_FOLDER_KEY, tidyState.folder);
  closeTidyFolderPicker();
  renderTidy();
  showToast('已选择：' + tidyState.folder.name, 'success');
}
...
function tidyUpdateTask(id, patch){
  var list = tidyTasks();
  for (var i = 0; i < list.length; i++){
    if (list[i].id === id){ for (var k in patch) if (Object.prototype.hasOwnProperty.call(patch, k)) list[i][k] = patch[k]; break; }
  }
  tidySaveTasks(list);
}
...
function openTidyChat(taskId){
  if (taskId){
    ...
    tidyState.chatDraft = [
      ...
      { role: 'ai', text: '① 点右上角「获取目录树」，把当前文件夹的结构读出来；② 告诉我你想怎么整理（比如「把文件名里的站点水印清掉」「番号统一大写」「按剧集分季」）；③ 聊满意后点下方「确认结果」，我才会生成整理清单。' }
    ];
  }
  tidyState.tree = '';
...
var tidyPlanRegistry = {};   /* mi（消息下标）→ 该卡片对应的 plan 对象；确保「导出」抓的永远和卡片显示的是同一份 */
  ...
  box.innerHTML = tidyChatMsgs().map(function (m, mi){
    ...
    if (m.role === 'plan'){
      /* 清单不铺在聊天里，收成一张可点的小卡片（像附件文件）：点开 = 预览并执行。
         卡片带上自己在消息里的位置 mi —— 一段对话出过多批清单时，每张卡都能回看自己的那份。
         同时把 plan 记进注册表，导出按钮直接按 mi 取，绝不会错位成别的批次。 */
      tidyPlanRegistry[mi] = m.plan;
      var n = (m.plan && m.plan.items) ? m.plan.items.length : (m.count || 0);
      var total = (m.plan && m.plan.total) ? m.plan.total : n;
      var more = total > n ? (total - n) : 0;
      var sub = more ? ('共 ' + total + ' 项，还有 ' + more + ' 项未列出 · 点开预览') : '点击预览，可逐项改名后执行';
      return '<div class="tidy-msg plan" onclick="tidyAiPlanOpen(' + mi + ')">' +
        '<span class="tp-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6.5A2.5 2.5 0 0 0 4 4.5v15A2.5 2.5 0 0 0 6.5 22h11a2.5 2.5 0 0 0 2.5-2.5V8z"/><path d="M14 2v6h6"/><path d="M8 13h8M8 17h5"/></svg></span>' +
        '<span class="tp-body"><b class="tp-tx">整理方案 · ' + n + ' 项</b><small class="tp-sub">' + sub + '</small></span>' +
        '<button class="tp-dl" onclick="tidyPlanExport(event,' + mi + ')">导出</button>' +
        '<span class="tp-go"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg></span>' +
        '</div>';
    }
    /* 自动补齐这类消息是 App 代发的，按系统提示样式显示，不伪装成用户自己说的话 */
    var cls = m.role === 'me' ? (m.auto ? 'sys' : 'me') : (m.role === 'ai' ? 'ai' : 'sys');
    var txt = String((m.auto && m.disp) ? m.disp : (m.text == null ? '' : m.text));
    if (!txt.trim()) return '';                              /* 空消息不渲染，避免出现一条空白行 */
    /* AI 回复里的 ```json 清单不进气泡（清单另以卡片呈现），气泡只留人话 */
    if (m.role === 'ai' && typeof TidyCore !== 'undefined' && TidyCore.stripJsonBlock){
      txt = TidyCore.stripJsonBlock(txt);
      if (!txt) return '';
    }
...
function tidyTreePreview(mi){
  ...
  var lines = m.lines || 0;
  var title = document.getElementById('tidyTreeTitle');
  var pre = document.getElementById('tidyTreeText');
  if (title) title.textContent = '目录树 · ' + lines + ' 行';
  if (pre) pre.textContent = text;
  tidySheetOpen('tidyTreeMask', 'tidyTreeSheet');
}
function closeTidyTreePreview(){
  tidySheetClose('tidyTreeMask', 'tidyTreeSheet');
}
...
/* 真实树 → 文本。不再走「拍平成导出格式 → 再启发式解析」的老路（那次来回会靠
   「有无子节点」反推目录、靠缩进猜层级，文件/空目录会误判、层级猜错兄弟全挂错父级）。
   递归列目录时手里就有真实 dir 标记（来自 115 接口），直接渲染，永不变形。
   规模自适应：默认完整渲染（不按深度折叠）；只有大到超出字符上限，才按节点预算逐步收紧。 */
function tidyTreeFromNode(root){
  var T = (typeof TidyCore !== 'undefined') ? TidyCore : null;
  if (!T || !T.renderTreeText) return { text: '', count: 0 };
  var caps = [999999, 4000, 2500, 1600, 1000, 700, 500, 350, 250, 160, 100, 60];
  var text = '';
  for (var i = 0; i < caps.length; i++){
    var tr = T.trimTree(root, { maxDepth: TIDY_TREE_MAX_DEPTH + 1, maxNodes: caps[i] });
    text = T.renderTreeText(tr, { maxLines: 4000 }) || '';
    if (text.length <= TIDY_TREE_CHAR_CAP) break;
  }
  if (text.length > TIDY_TREE_CHAR_CAP) text = text.slice(0, TIDY_TREE_CHAR_CAP) + '\n…（已截断）';
  return { text: text, count: T.countNodes(root) };
}
...
function tidyChatSend(){
  ...
  if (!text) return;
  inp.value = '';
  tidyChatSendText(text);
}
/* 自动补齐：AI 分批给清单时，剩余只剩少许就不再让用户手点「继续」——
   自动再问一轮并并进同一张卡片。剩余很多（或连补两轮还没完）才交回人工决定。 */
var TIDY_AUTO_MERGE_MAX = 20;    /* 剩余不超过这么多条：自动补齐，不再显示「继续出下一批」 */
var TIDY_AUTO_ROUNDS_MAX = 2;    /* 最多自动补两轮，防模型一直报「还有几项」时反复发请求 */

/* 真正发一条（输入框「发送」与输入框上方「确认结果」共用）
   opts.auto / opts.mergeMi：App 代发的「继续」，结果并进 mergeMi 那张已有方案卡 */
function tidyChatSendText(text, opts){
  opts = opts || {};
  text = String(text || '').trim();
  if (!text) return;
  if (tidyState.chatSending){ showToast('AI 正在回答，稍等一下', 'info'); return; }
  var msgs = tidyChatMsgs();
  msgs.push({ role: 'me', text: text, auto: !!opts.auto, disp: opts.disp || '' });
  ...
  tidyChatSetMsgs(msgs);
  tidyUpdateTask(tidyState.chatId, { detail: (opts.auto && opts.disp) ? opts.disp : text.slice(0, 40) });
  renderTidyChat();
  ...
  var autoNext = null;   /* 需要自动补齐时，等本轮收尾（状态复位）后再发下一轮 */
  tidyAiRequest().then(function (res){
    /* 完整回复照旧落盘（下一轮要带回去给 AI 当上下文），只是渲染时把 JSON 块藏起来 */
    ...
    if (!items.length){ tidyTaskStep(cid, 'chat', 'ok', ''); return; }
    /* 清单可能很长，落盘前先截断，避免把 localStorage 撑爆（total 记原总数，用于「还有 N 项」） */
    var total = (res.plan.total && res.plan.total > items.length) ? res.plan.total : items.length;
    var capped = items.slice(0, 800);
    var mi, all = tidyChatMsgs();
    if (opts.mergeMi != null && all[opts.mergeMi] && all[opts.mergeMi].role === 'plan' && all[opts.mergeMi].plan){
      /* 自动补齐的这一批：并进原来那张卡，聊天里不新开卡片 */
      all[opts.mergeMi].plan.items = all[opts.mergeMi].plan.items.concat(capped);
      all[opts.mergeMi].plan.total = Math.max(total, all[opts.mergeMi].plan.items.length);
      all[opts.mergeMi].count = all[opts.mergeMi].plan.items.length;
      tidyChatSetMsgs(all);
      renderTidyChat();
      mi = opts.mergeMi;
    } else {
      tidyChatAppend({ role: 'plan', text: '整理清单', count: capped.length, plan: { root: res.plan.root || '', total: total, items: capped } });
      mi = tidyChatMsgs().length - 1;
    }
    var got = tidyChatMsgs()[mi].plan.items.length;
    var want = tidyChatMsgs()[mi].plan.total;
    var left = want > got ? (want - got) : 0;
    tidyTaskStep(cid, 'chat', 'ok', '');
    tidyTaskStep(cid, 'plan', 'ok', got + ' 项');
    var round = opts.round || 0;
    if (left && left <= TIDY_AUTO_MERGE_MAX && round < TIDY_AUTO_ROUNDS_MAX){
      /* 只剩几项：静默再要一轮，别为两三条让用户专门去点一次按钮 */
      autoNext = { mi: mi, round: round + 1, left: left };
      showToast('已生成 ' + got + ' 项，正在自动补齐剩余 ' + left + ' 项…', 'info');
    } else {
      showToast(left
        ? ('已生成 ' + got + ' 项（共 ' + want + ' 项），可继续出下一批')
        : ('AI 已生成整理清单（' + got + ' 项）'), 'success');
    }
  }, function (e){
    var m = (e && e.message) || '网络错误';
    ...
    tidyConfirmLabel();
    if (autoNext){
      tidyChatSendText('继续，出后面还没列的清单（已经列过的前 ' + tidyChatMsgs()[autoNext.mi].plan.items.length + ' 条不用重复）。',
        { auto: true, mergeMi: autoNext.mi, round: autoNext.round, disp: '自动补齐剩余 ' + autoNext.left + ' 项…' });
    }
  });
}
/* 取最近一张清单卡片里的 plan */
function tidyLastPlan(){
  var msgs = tidyChatMsgs();
  for (var i = msgs.length - 1; i >= 0; i--){
    if (msgs[i] && msgs[i].role === 'plan' && msgs[i].plan) return msgs[i].plan;
  }
  return null;
}
/* 输入框上方那颗按钮的文案：还有没列出的条目时，变成「继续出下一批」 */
function tidyConfirmLabel(){
  var btn = document.getElementById('tidyConfirmBtn');
  if (!btn) return;
  btn.disabled = !!tidyState.chatSending;
  var p = tidyLastPlan();
  var n = (p && p.items) ? p.items.length : 0;
  var total = (p && p.total) ? p.total : n;
  var more = total > n ? (total - n) : 0;
  btn.textContent = more ? ('继续出下一批（还有 ' + more + ' 项）') : '确认结果，生成整理清单';
}
/* 「确认结果」：点了才让 AI 出清单 —— 平时只聊天，不吐 JSON */
function tidyChatConfirm(){
  if (tidyState.chatSending){ showToast('AI 正在回答，稍等一下', 'info'); return; }
  if (!tidyState.folder){ showToast('请先选择目标文件夹', 'error'); return; }
  if (!tidyCurrentTree()){ showToast('请先点右上角「获取目录树」', 'error'); return; }
  var hasMe = tidyChatMsgs().some(function (m){ return m && m.role === 'me'; });
  if (!hasMe){ showToast('先说说你想怎么整理，再点确认', 'error'); return; }
  var p = tidyLastPlan();
  var n = (p && p.items) ? p.items.length : 0;
  var total = (p && p.total) ? p.total : n;
  tidyChatSendText(total > n
    ? ('继续，出后面的清单（已经列过的前 ' + n + ' 条不用重复）。')
    : '确认结果，出清单。');
}
/* 把清单导出成 .json 文件（留档 / 给外部 AI 复用）；不支持下载时退回「复制到剪贴板」 */
function tidyPlanExport(ev, mi){
  if (ev && ev.stopPropagation) ev.stopPropagation();
  if (ev && ev.preventDefault) ev.preventDefault();
  /* 导出跟随被点的那张卡片：优先用渲染期建好的 mi→plan 注册表（与卡片显示严格一致），
     再退到 msgs[mi]，最后才退回最近一张 —— 避免多批次时导出错位成别的清单 */
  var msgs = tidyChatMsgs(), p = null;
  if (mi != null && tidyPlanRegistry[mi]) p = tidyPlanRegistry[mi];
  if (!p && mi != null && msgs[mi] && msgs[mi].role === 'plan' && msgs[mi].plan) p = msgs[mi].plan;
  if (!p) p = tidyLastPlan();
  if (!p || !p.items || !p.items.length){ showToast('没有可导出的清单', 'error'); return; }
  var txt = (typeof TidyCore !== 'undefined' && TidyCore.planToJson) ? TidyCore.planToJson(p) : JSON.stringify(p, null, 2);
  var safe = String(tidyFolderName() || '未命名').replace(/[\\/:*?"<>|]/g, '_');
  var name = '整理清单_' + safe + '.json';
  var done = false;
  try {
  ...
  if (done){ showToast('清单已导出：' + name, 'success'); return; }
  try {
    navigator.clipboard.writeText(txt);
    showToast('已复制清单内容（可粘贴进文件保存）', 'success');
  } catch (e2){ showToast('导出失败', 'error'); }
}
...
function renderTidyRules(){
  ...
  var btn = document.getElementById('tidyRunBtn');
  if (btn) btn.classList.toggle('disabled', !tidyState.ruleId);
}
function tidyToggleRule(id){
...
/* 解析相对路径到 115 目录 cid。
   createIfMissing=true（默认）：沿路径逐段找已有子目录，找不到就当场 files/add 建好再往下走
   —— 这样「移动到新文件夹」也能成立（AI/规则整理里想把文件挪进尚不存在的目录时不再报「找不到文件」）。
   onCreated(段名) 在每新建一个目录段时回调（用于统计新建了几个夹、给个提示）。
   建夹每步都走 c115Call 写闸（与改名/移动同一套节流），命中已有目录则零额外请求。 */
function tidyResolveDir(rootCid, relPath, createIfMissing, onCreated){
  ...
  segs.forEach(function (seg){
    chain = chain.then(function (cid){
      if (!cid) return null;
      return auto115FindDir(cid, seg).then(function (hit){
        if (hit) return hit.cid;
        if (!createIfMissing) return null;
        /* 不存在 → 建一个，拿回新 cid 继续往下走 */
        return auto115Post('https://webapi.115.com/files/add',
          'pid=' + encodeURIComponent(cid) + '&cname=' + encodeURIComponent(seg)
        ).then(function (res){
          var d = (res && res.d) || {}, dd = d.data || d;
          var newCid = String((dd && (dd.cid || dd.file_id || dd.id)) || '');
          if (!newCid) throw new Error('建文件夹「' + seg + '」失败');
          if (typeof onCreated === 'function') onCreated(seg);
          return newCid;
        });
      });
    });
...
function tidyJsonSnapshots(rootCid, entries){
  ...
  dirs.forEach(function (d){
    chain = chain.then(function (){
      return tidyResolveDir(rootCid, d, false).then(function (cid){
        dirCid[d] = cid || null;
...
/* —— AI 整理：把 AI 给的清单落到「定位 → 预览 → 执行」这条同动线上 ——
   ...
   mi = 被点卡片在消息里的位置：一段对话出过多批清单时每张卡都能回看自己的那份，不是只认最新一张。 */
function tidyAiPlanOpen(mi){
  var msgs = tidyChatMsgs(), plan = null;
  if (mi != null && msgs[mi] && msgs[mi].role === 'plan' && msgs[mi].plan) plan = msgs[mi].plan;
  else {
    for (var i = msgs.length - 1; i >= 0; i--){
  ...
  /* 点了立刻开弹窗：先给加载态，定位在后台跑，结果回来再逐步铺列表（tidyAiRun 内核对 token） */
  var tok = tidyPvBegin();
  tidyPreviewLoading('正在定位清单里的文件…');
  tidySheetOpen('tidyPreviewMask', 'tidyPreviewSheet');
  tidyAiRun(plan, { token: tok });
}
...
function tidyAiRun(plan, opt){
  /* opt.token：弹窗已经先一步打开（点卡片直接进预览）。定位结果回来时先核对 token，
     用户中途关掉弹窗就不再回填、也不重新开窗；失败也只在弹窗还开着时负责关它。 */
  opt = opt || {};
  ...
  if (opt.token == null) showToast('正在定位清单里的文件…', 'info');
  ...
  tidyJsonSnapshots(tidyState.folder.cid, entries).then(function (snap){
    ...
    if (!res.ops.length){
      ...
      tidyState.pendingTask = null; tidyState.pendingKind = null;
      if (opt.token != null && tidyPvAlive(opt.token)) tidySheetClose('tidyPreviewMask', 'tidyPreviewSheet');
...
function tidyWatermarks(){
  if (!tidyState.wm){
    var saved = tidyLoad(TIDY_WM_KEY, null);
    tidyState.wm = (saved && saved.length) ? saved : (typeof TidyCore !== 'undefined' ? TidyCore.defaultWatermarks() : []);
  }
  return tidyState.wm;
}
function openTidyWatermarkSheet(){ renderTidyWm(); tidySheetOpen('tidyWmMask', 'tidyWmSheet'); }
function closeTidyWatermarkSheet(){ tidySheetClose('tidyWmMask', 'tidyWmSheet'); }
function renderTidyWm(){
  var box = document.getElementById('tidyWmList');
  if (!box) return;
...
function tidyWmReset(){
  if (typeof TidyCore === 'undefined') return;
  tidyState.wm = TidyCore.defaultWatermarks();
  renderTidyWm();
  showToast('已恢复默认水印', 'success');
}
function tidyWmSave(){
  var list = tidyWatermarks().filter(function (w) { return w && w.src; });
  tidyState.wm = list;
  tidyStore(TIDY_WM_KEY, list);
  closeTidyWatermarkSheet();
  showToast('水印已保存（' + list.length + ' 条）', 'success');
}

/* —— 预览 + 执行 —— */
function tidyPreviewRun(){
...
function closeTidyPreview(){
  tidyPvToken++;   // 作废所有在途的预览回填（点卡片后正在后台定位的那份，回来时发现 token 变了就不再开窗）
...
/* —— 预览弹窗的「先开窗后加载」支撑 ——
   tidyPvBegin 发新票（旧的一律作废），tidyPvAlive 验票：异步结果回来时只有票还有效才回填。 */
var tidyPvToken = 0;
function tidyPvBegin(){ return ++tidyPvToken; }
function tidyPvAlive(tok){ return tok === tidyPvToken; }
/* 预览弹窗加载态：点开立刻有反馈，定位在后台跑，避免「点了没反应」的空窗 */
function tidyPreviewLoading(hint){
...
function renderTidyPreview(ops){
  ...
  /* 首屏先铺 20 行，剩下的分帧补上：弹窗一开就有内容，长列表不再整块卡一下 */
  var CHUNK = 20;
  var made = Math.min(CHUNK, show.length);
  box.innerHTML = show.slice(0, made).map(function (_, i){ return rowAt(i); }).join('') + notes;
  (function more(){
    if (made >= show.length) return;
...
function tidyPreviewEdit(idx){
  ...
  var o = ops[idx];
  if (!o) return;
  tidyState.editIdx = idx;
  var oldDirEl = document.getElementById('tidyNameOldDir');
  var oldEl = document.getElementById('tidyNameOld');
  var newDirInp = document.getElementById('tidyNameNewDir');
  var inp = document.getElementById('tidyNameInput');
  var title = document.getElementById('tidyNameTitle');
  if (oldDirEl){ oldDirEl.textContent = o.oldDir || ''; oldDirEl.classList.toggle('empty', !o.oldDir); }
  if (oldEl) oldEl.textContent = o.orig || '';
  if (newDirInp) newDirInp.value = (o.newDir || '');
  if (title) title.textContent = (o.op === 'move') ? '修改名称与路径（并移动到新文件夹）' : '修改名称与路径';
  if (inp) inp.value = o.name || '';
  tidySheetOpen('tidyNameMask', 'tidyNameSheet');
  /* 弹窗动画结束后聚焦并把光标放到末尾，方便直接改 */
  setTimeout(function (){
    try { if (inp){ inp.focus(); var n = inp.value.length; inp.setSelectionRange(n, n); } } catch (e) {}
  }, 320);
}
function closeTidyNameEdit(){
  tidySheetClose('tidyNameMask', 'tidyNameSheet');
  tidyState.editIdx = null;
}
function tidyNameSave(){
  var i = tidyState.editIdx;
  var ops = tidyState.preview || [];
  if (i == null || !ops[i]){ closeTidyNameEdit(); return; }
  var newDirInp = document.getElementById('tidyNameNewDir');
  var inp = document.getElementById('tidyNameInput');
  var o = ops[i];
  var oldDir = String(o.oldDir || '').replace(/\/+$/, '').replace(/^\/+/, '');
  /* 新路径：去掉首尾斜杠；为空＝留在原目录 */
  var nd = newDirInp ? String(newDirInp.value || '').trim().replace(/\/+$/, '').replace(/^\/+/, '') : oldDir;
  if (/[\\:*?"<>|]/.test(nd)){ showToast('路径不能包含 \\ : * ? " < > |', 'error'); return; }
  var v = inp ? String(inp.value || '').trim() : '';
  if (!v){ showToast('名称不能为空', 'error'); return; }
  if (/[\\/:*?"<>|]/.test(v)){ showToast('名称不能包含 \\ / : * ? " < > |', 'error'); return; }
  /* 路径变了 → 变成移动（执行时再解析目标目录 cid）；没变 → 原地改名 */
  if (nd !== oldDir){
    o.op = 'move';
    o.newDir = nd;
    o.toDir = nd;
    o.toCid = '__resolve__';    // 标记：执行前需按新路径重新向 115 解析 cid
  } else {
    o.op = (o.op === 'move') ? 'rename' : o.op;
    o.newDir = oldDir;
    o.toDir = undefined;
    o.toCid = undefined;
  }
  if (v !== o.name) o.name = v;
  closeTidyNameEdit();
  renderTidyPreview(ops);
}
function tidyExecute(){
  var ops = tidyState.preview;
  ...
  /* 0 阶段：move 条目的目标 cid 标了 '__resolve__'（来源有两处：①用户在改名弹窗改了新路径；
     ②清单里目标目录在快照时还不存在）→ 执行前按新路径向 115 解析真实 cid；
     沿途缺失的目录段当场建好（tidyResolveDir 负责）。同一条目标路径只解析一次，避免重复建夹。 */
  function resolvePendingMoves(){
    var pending = moveOps.filter(function (o){ return o.toCid === '__resolve__'; });
    if (!pending.length) return Promise.resolve();
    var rootCid = (tidyState.folder && tidyState.folder.cid) || '0';
    var cache = {}, created = 0;
    function resolveOne(path){
      if (cache[path]) return cache[path];
      var p = tidyResolveDir(rootCid, path, true, function (){ created++; });
      cache[path] = p;
      return p;
    }
    return Promise.all(pending.map(function (o){
      return resolveOne(o.newDir).then(function (cid){
        o.toCid = cid || null;
      }).catch(function (){ o.toCid = null; });
    })).then(function (){
      if (created) showToast('已自动新建 ' + created + ' 个文件夹', 'success');
    });
  }
  ...
  function runMoveGroups(){
    ...
    /* 建夹失败 / 解析不到的 move：toCid 仍为 null，单独计失败，避免被静默吞掉、误导成「全成功」 */
    var badGroup = groups['null'] || groups[undefined] || groups['undefined'];
    if (badGroup && badGroup.length) failed += badGroup.length;
    var keys = Object.keys(groups).filter(function (k){ return k && k !== 'null' && k !== 'undefined'; });
    return new Promise(function (resolve){
      var gi = 0;
      (function next(){
        ...
        var valid = group.filter(function (o){ return o && o.fid && o.toCid; });
  ...
  resolvePendingMoves().then(runRenameBatches).then(runMoveGroups).then(finishExec);
}
...
function tbmConfirmOrganize(){
  ...
  /* v338：磁力库多磁力仅下载（2 步表）；自动整理需按每条磁力定位落点，尚未支持，先明确拦住不静默走坏 */
  if (Auto115Core.isMultiMagnet(t)){
    showToast('多磁力任务的自动整理还没支持，请先在对应影片/剧集页整理', 'info');
    return;
  }
