...
(async () => {
  ...
  script = {
    ...
    'files?cid=DIR999': { state: true, data: [
      { fid: 'F1', n: 'a.mp4', s: 100 },                           // 极小视频 → 当广告删
      { fid: 'F2', n: 'movie.mkv', s: 800 * 1024 * 1024 },         // 主视频（最大，800MB）
      { fid: 'F3', n: 'sample.mp4', s: 5000 },                     // sample → 删
      { fid: 'F4', n: 'another.movie.mkv', s: 600 * 1024 * 1024 }, // 大体积其他片 → 合集保护，保留
      { cid: 'SUB1', n: 'subs', t: NOW_SEC }                       // 普通名子文件夹 → 保留
    ] },
  ...
  assert(delCall && delCall.body.indexOf('F3') >= 0 && delCall.body.indexOf('F1') >= 0, '删了 sample.mp4 与小体积广告视频 a.mp4（100B < 50MB）');
  assert(delCall && delCall.body.indexOf('F4') < 0 && delCall.body.indexOf('SUB1') < 0, '合集保护：大体积其他视频 another.movie.mkv 与子文件夹 subs 均保留不删');
  assert(t.keptOtherVideos === 1, '记录保留的其他视频数 = 1（F4）');
  ...
  /* 2c. 合集场景（赌神 2部全）：赌神2 不再被收编/删除，重复副本判重只留最大，sample 删除 */
  const GB = 1024 * 1024 * 1024;
  script['files?cid=DIRCS'] = { state: true, data: [
    { fid: 'G1', n: '赌神.1080p.国粤双语.BD中字.mp4', s: 2.5 * GB },
    { fid: 'G2', n: '赌神.1080p.国粤双语.BD中字(1).mp4', s: 2.5 * GB },   // 与 G1 同大小 → 判重删
    { fid: 'G3', n: '赌神2.1994.1080p.mp4', s: 2.5 * GB },                // 赌神2 → 合集保护，保留
    { fid: 'G4', n: 'sample.mp4', s: 5000 }                                // sample → 删
  ] };
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = '';
  const tcs = { id: 'tcs', magnet: 'magnet:?xt=urn:btih:collection', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2 };
  doc.tasks.unshift(tcs);
  ctx.auto115GetStep(tcs, 'mkdir').state = 'ok';
  tcs.offlineDirCid = 'DIRCS'; tcs.offlineDirName = '赌神 2部全';
  calls.length = 0;
  await ctx.auto115StepMove(tcs);
  assert(ctx.auto115GetStep(tcs, 'move').state === 'ok', '合集：move = ok');
  assert(tcs.keepFids && tcs.keepFids.length === 1 && tcs.keepFids[0] === 'G1', '合集：赌神2 不命中标题，重复副本判重后只留 G1（不再 cd1/cd2）');
  const delCS = calls.filter(c => c.url.indexOf('rb/delete') >= 0).map(c => c.body || '').find(b => b.indexOf('pid=DIRCS') >= 0) || '';
  assert(delCS && delCS.indexOf('G2') >= 0 && delCS.indexOf('G4') >= 0, '合集：重复副本 G2 与 sample G4 进删除清单');
  assert(delCS && delCS.indexOf('G3') < 0 && delCS.indexOf('G1') < 0, '合集：赌神2(G3) 合集保护保留、主视频 G1 不删');
  assert(tcs.keptOtherVideos === 1, '合集：保留其他视频数 = 1');
  doc.dvdId = 'IPX-486'; doc.filmTitle = '测试影片'; doc.originalTitle = ''; doc.year = '';

  /* 2d. V3：平铺影片重复整理——根目录已有同名同大小视频 → 跳过，不再造 .A 副本
     （v265 起改名带清晰度：原文件 赌神.1080p.mp4 → 目标名 赌神.1080p.mp4，判重按新目标名比对） */
  script['files?cid=3311283881428122938'] = { state: true, data: [
    { fid: 'R1', n: '赌神.1080p.mp4', s: 2.5 * GB }                        // 根目录已有同名同大小 → 同一文件
  ] };
  script['files?cid=DIRTMP'] = { state: true, data: [] };                  // 临时夹已空 → 会顺带删除
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = ''; doc.nfoUploaded = false;
  const tflat = { id: 'tflat', magnet: 'magnet:?xt=urn:btih:flatdup', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2, videoFid: 'G1', videoName: '赌神.1080p.mp4', videoSize: 2.5 * GB, keepFids: ['G1'], offlineDirCid: 'DIRTMP', offlineDirName: '赌神' };
  doc.tasks.unshift(tflat);
  calls.length = 0;
  await ctx.auto115StepRenameFlat(tflat);
  assert(ctx.auto115GetStep(tflat, 'rename').state === 'ok' && ctx.auto115GetStep(tflat, 'rename').msg.indexOf('跳过重复') >= 0, '平铺判重：根目录同名同大小 → 跳过（' + ctx.auto115GetStep(tflat, 'rename').msg + '）');
  assert(!calls.some(c => c.url.indexOf('files/edit') >= 0), '平铺判重：未发起改名请求（无 files/edit）');
  doc.dvdId = 'IPX-486'; doc.filmTitle = '测试影片'; doc.originalTitle = ''; doc.year = ''; doc.nfoUploaded = false;

  /* 2e. 赌神合集·平铺（v263 修丢片）：move 保住赌神2 → rename 平铺时必须把赌神2 一起移出临时夹，再删夹 */
  script['files?cid=DIRCS2'] = { state: true, data: [
    { fid: 'G1', n: '赌神.1080p.国粤双语.BD中字.mp4', s: 2.5 * GB },
    { fid: 'G3', n: '赌神2.1994.1080p.mp4', s: 2.5 * GB }                 // 赌神2 → 合集保护
  ] };
  script['files?cid=3311283881428122938'] = { state: true, data: [] };    // 云下载根目录为空
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = ''; doc.nfoUploaded = false;
  const tcs2 = { id: 'tcs2', magnet: 'magnet:?xt=urn:btih:cs2', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2 };
  doc.tasks.unshift(tcs2);
  ctx.auto115GetStep(tcs2, 'mkdir').state = 'ok';
  tcs2.offlineDirCid = 'DIRCS2'; tcs2.offlineDirName = '赌神合集';
  calls.length = 0;
  await ctx.auto115StepMove(tcs2);
  ...
  script['files?cid=DIRGOD'] = { state: true, data: [] };                 // 已存在的「赌神」文件夹（空）
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = ''; doc.nfoUploaded = 1;   // nfoUploaded → 需要文件夹
  const tmg = { id: 'tmg', magnet: 'magnet:?xt=urn:btih:mg', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2,
    videoFid: 'G1', videoName: '赌神.1080p.mp4', videoSize: 2.5 * GB, keepFids: ['G1'],
    offlineDirCid: 'DIRTMP2', offlineDirName: '赌神 2部全',
    forcedMergeCid: 'DIRGOD', finalDirName: '赌神',
    keptOtherVideos: 1, keptOthers: [{ fid: 'G3', name: '赌神2.1994.1080p.mp4', size: 2.5 * GB }] };
  doc.tasks.unshift(tmg);
  calls.length = 0;
  ...
  /* 2g. 赌神合集·并入判重跳过路径：主视频已在目标夹 → 赌神2 仍要移出临时夹再删夹 */
  script['files?cid=DIRGOD'] = { state: true, data: [ { fid: 'R1', n: '赌神.1080p.mp4', s: 2.5 * GB } ] };
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = ''; doc.nfoUploaded = 1;
  const tmg2 = { id: 'tmg2', magnet: 'magnet:?xt=urn:btih:mg2', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2,
    videoFid: 'G1', videoName: '赌神.1080p.mp4', videoSize: 2.5 * GB, keepFids: ['G1'],
    offlineDirCid: 'DIRTMP2', offlineDirName: '赌神 2部全',
    forcedMergeCid: 'DIRGOD', finalDirName: '赌神',
    keptOtherVideos: 1, keptOthers: [{ fid: 'G3', name: '赌神2.1994.1080p.mp4', size: 2.5 * GB }] };
  doc.tasks.unshift(tmg2);
  calls.length = 0;
  ...
  script['files?cid=DIRQ'] = { state: true, data: [
    { fid: 'Q1', n: '赌神.1080p.国粤双语.mkv', s: 2.5 * GB },
    { fid: 'Q2', n: '赌神.720p.mkv', s: 1.2 * GB },
    { fid: 'Q3', n: '赌神.480p.mkv', s: 0.6 * GB }
  ] };
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = '';
  const tqual = { id: 'tqual', magnet: 'magnet:?xt=urn:btih:quality', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2 };
  doc.tasks.unshift(tqual);
  ctx.auto115GetStep(tqual, 'mkdir').state = 'ok';
  tqual.offlineDirCid = 'DIRQ'; tqual.offlineDirName = '赌神';
  calls.length = 0;
  ...
  script['files?cid=3311283881428122938'] = { state: true, data: [] };
  doc.dvdId = ''; doc.filmTitle = '赌神'; doc.originalTitle = ''; doc.year = '1989'; doc.nfoUploaded = false;
  const tqf = { id: 'tqf', magnet: 'magnet:?xt=urn:btih:qtag', magnetTitle: '赌神', steps: ctx.auto115NewSteps(), createdAt: Date.now(), fv: 2,
    noFolder: true, videoFid: 'Q1', videoName: '赌神.1080p.mkv', videoSize: 2.5 * GB, keepFids: ['Q1'] };
  doc.tasks.unshift(tqf);
  calls.length = 0;
  ...
  assert(ctx.auto115GetStep(tt, 'rename').state === 'ok' && ctx.auto115GetStep(tt, 'rename').msg.indexOf('整理测试片.2020.1080p.mkv') >= 0, '整理任务改视频名（追加清晰度）：' + ctx.auto115GetStep(tt, 'rename').msg);
  ...
  /* 11. 多磁力任务流（v336）回归：电影单文件夹 / 系列改名 / 剧集分季+未识别夹 / 数据模型 */
  /* 隔离：专属 doc，避免 advanceQueue 捡起前面用例残留的任务 */
  const mkMultiDoc = (type, title, extra) => Object.assign({ filmId: 'filmMulti', type: type, filmTitle: title, dvdId: '', originalTitle: '', year: '', tasks: [] }, extra || {});
  ctx.auto115RunningId = ''; ctx.auto115ExecDoc = null;

  // 11e. 数据模型：isMultiMagnet / magnets 归一化 / 任务标题
  assert(ctx.Auto115Core.isMultiMagnet({ magnets: [{}, {}] }) === true, '多磁力：isMultiMagnet 有 magnets 数组 → true');
  assert(ctx.Auto115Core.isMultiMagnet({ magnet: 'magnet:?xt=urn:btih:x' }) === false, '多磁力：旧单磁力 → false');
  assert(ctx.Auto115Core.isMultiMagnet({}) === false, '多磁力：空任务 → false');
  const legN = ctx.Auto115Core.magnets({ magnet: 'magnet:?xt=urn:btih:leg', magnetTitle: '旧磁力' });
  assert(legN.length === 1 && legN[0].magnet === 'magnet:?xt=urn:btih:leg', '多磁力：旧单磁力归一化为 1 条（magnet 保留）');
  const mulN = ctx.Auto115Core.magnets({ magnets: [ { magnet: 'm1', title: 'A', state: 'done' }, { magnet: 'm2', title: 'B', state: 'done' } ] });
  assert(mulN.length === 2, '多磁力：magnets 返回原数组长度');
  const tOff = { magnets: [{}, {}, {}] };
  ...
  const tMov = { id: 'tmov1', type: 'offline', multi: true, magnets: [
    ...
    { magnet: 'magnet:?xt=urn:btih:bb', title: '电影B.1080p', dirCid: 'DIRB', dirName: '电影B.1080p', state: 'done' },
  ...
  assert(ctx.auto115GetStep(tMov, 'rename').state === 'ok', '电影多磁力：rename 步 = ok（' + ctx.auto115GetStep(tMov, 'rename').msg + '）');
  ...
  const tMovS = { id: 'tmovs', type: 'offline', multi: true, magnets: [
    { magnet: 'magnet:?xt=urn:btih:sa', title: 'Series.Part1.1080p', dirCid: 'DSA', dirName: 'Series.Part1.1080p', state: 'done' },
    { magnet: 'magnet:?xt=urn:btih:sb', title: 'Series.Part2.1080p', dirCid: 'DSB', dirName: 'Series.Part2.1080p', state: 'done' }
  ], steps: ctx.auto115NewSteps('multi'), createdAt: Date.now() };
  script = {
    'files?cid=3311283881428122938': { state: true, data: [] },
  ...
  const tTvSplit = { id: 'ttvm', type: 'offline', multi: true, magnets: [
    { dirCid: 'D1', dirName: 'Show.S01E01.mkv', state: 'done' },
